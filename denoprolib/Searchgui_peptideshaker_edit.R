...
for (k in 1:length(trinity_output)) {
    ...
    system(paste("java -cp ",searchgui," eu.isas.searchgui.cmd.SearchCLI -spectrum_files ",spectra_files," -id_params ",i,".par -output_folder ", spectra_files," -xtandem 1 -msgf 1 -tide 1  -output_default_name ",i,"_searchgui.out",sep=''))
    }
...
for (k in 1:length(sgui_out1)) {
    i<-sgui_out1[k]
    j<-sgui_out2[k]
    system(paste("java -cp ",peptideshaker," eu.isas.peptideshaker.cmd.PeptideShakerCLI -experiment ",i," -sample ",i," -replicate 1 -identification_files ",spectra_files,"/",i," -spectrum_files ",spectra_files,"/",j," -out ",spectra_files,"/",i,".cpsx",sep=''))
    }

pshaker_out<-list.files(pattern="*.cpsx$")
for(k in 1:length(pshaker_out)) {
    i=pshaker_out[k]
    system(paste("java -cp ",peptideshaker," eu.isas.peptideshaker.cmd.ReportCLI -in ",spectra_files,"/",i," -out_reports ",spectra_files," -reports 3,6,9",sep=''))
    }
